...
Set `save_freq` to `-1` to disable periodic checkpoint saves. Tinker still saves a final checkpoint when training ends, in both synchronous and async training.
